...
namespace Weathernews.Sensor
{
    ...
    public class WxBeacon2 : IDisposable
    {
        ...
        private static async Task<byte[]> ReadCharacteristicAsync(BluetoothLEDevice device, Guid serviceUuid, Guid characteristicUuid)
        {
            ...
            var service = serviceFinder.Services.Single(s => s.Uuid == serviceUuid) ?? throw new Exception("サービスが見つかりませんでした");
            ...
            var characteristic = characteristicFinder.Characteristics.Single(c => c.Uuid == characteristicUuid) ?? throw new Exception("キャラクタリスティックが見つかりませんでした");
